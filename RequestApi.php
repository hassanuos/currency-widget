...
class RequestApi
{
    ...
    public static function getCacheExpiry(){
        $projectConfigs = parse_ini_file(self::PROJECT_PATH . "env.ini", true);
        return isset($projectConfigs['settings']['cache_expiry']) ? $projectConfigs['settings']['cache_expiry'] : 0;
    }
    /**
    ...
    public static function writeIntoJsonFile($jsonData){
        $path = self::PROJECT_PATH . "cache.json";
    ...
    public static function readCache(){
        $cache_file = self::PROJECT_PATH . "cache.json";
        if (file_exists($cache_file) && (filemtime($cache_file) > (time() - (int)self::getCacheExpiry()))) {
    ...
    public static function codeToCountry($code){
        ...
        return ( !$countryList[$code] ) ? $code : $countryList[$code];
    }
}
